...
export async function GET(
  ...
  const [walletRes, earlyBuysRes, alertsRes] = await Promise.all([
    ...
    supabaseAdmin.from("early_buys").select("*").eq("wallet_address", address).order("buy_timestamp", { ascending: false }),
    supabaseAdmin.from("wallet_alerts").select("*").eq("wallet_address", address).order("timestamp", { ascending: false }).limit(20),
  ]);
